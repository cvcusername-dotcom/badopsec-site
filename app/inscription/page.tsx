...
import { ArrowLeft, Lock, User, CheckCircle } from "lucide-react";
import { FadeIn } from "@/components/ui/FadeIn";
import { supabase } from "@/Lib/supabaseClient";

export default function InscriptionPage() {
  const [formData, setFormData] = useState({ email: "", username: "", password: "", confirmPassword: "", discordTag: "" });
  ...
  const getAuthErrorMessage = (error: { message: string; status?: number }) => {
    ...
    if (error.status === 422) {
      ...
      if (message.includes("password")) {
        return "Mot de passe invalide ou trop court. Utilisez au moins 6 caractères.";
      }
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (error) {
      ...
      if (error.status === 429 || message.toLowerCase().includes("trop de tentatives") || message.toLowerCase().includes("rate limit")) {
            ...
            <div className="w-full rounded-[2rem] border border-white/10 bg-white/5 p-8 shadow-[0_40px_120px_rgba(30,10,80,0.5)] backdrop-blur-xl">
              <div className="mb-8 flex flex-col items-center gap-4 text-center">
                <div className="grid place-items-center rounded-3xl bg-violet-500/10 p-5 shadow-[0_20px_60px_rgba(142,92,243,0.18)]">
                  <User className="w-8 h-8 text-violet-200" />
                </div>
              ...
              {isSubmitted ? (
                <div className="space-y-6 text-center py-10">
                  ...
                  <h2 className="text-2xl font-semibold">Compte créé !</h2>
                  <p className="text-sm text-muted-foreground">Ton compte a bien été créé. Vérifie ta boîte mail pour confirmer ton adresse avant de te connecter.</p>
                  <Link href="/connexion" className="inline-flex w-full items-center justify-center rounded-full bg-gradient-to-r from-violet-500 to-violet-400 px-6 py-4 text-sm font-semibold text-white shadow-[0_20px_60px_rgba(142,92,243,0.25)]">
                    Se connecter
                  </Link>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    <label htmlFor="username" className="mb-3 block text-xs font-semibold uppercase tracking-[0.35em] text-muted-foreground">
                      Nom d'utilisateur
                    </label>
                  ...
                  <div>
                    <label htmlFor="confirmPassword" className="mb-3 block text-xs font-semibold uppercase tracking-[0.35em] text-muted-foreground">
                  ...
                  <div>
                    <label htmlFor="discordTag" className="mb-3 block text-xs font-semibold uppercase tracking-[0.35em] text-muted-foreground">
